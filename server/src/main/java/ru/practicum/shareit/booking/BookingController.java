...
public class BookingController {
    ...
    List<BookingDto> getAllByBooker(@RequestHeader(name = "X-Sharer-User-Id") long userId,
        ...
        List<BookingDto> bookingsByBooker =
                bookingService.getAllByBooker(userId, getBookingState(state), PageableCreate.pageableCreate(from, size));
    ...
    List<BookingDto> getAllByOwner(@RequestHeader(name = "X-Sharer-User-Id") long userId,
                                   ...
                                   @RequestParam(name = "size", defaultValue = "10") int size) {
        List<BookingDto> bookingsByBooker =
                bookingService.getAllByOwner(userId, getBookingState(state), PageableCreate.pageableCreate(from, size));
